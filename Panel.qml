...
Panel {
  ...
  readonly property var chargeLimits: [80, 100]
  readonly property int chargeLimitEnd: Model.parseChargeLimitEnd(root.batteryInfo.threshold)
  readonly property bool chargeLimitSupported: root.chargeLimitReadOk || root.batteryInfo.threshold !== undefined
  property int chargeLimitActual: 100
  property bool chargeLimitReadOk: false
  ...
  function updateChargeLimit(raw) {
    ...
    if (isNaN(v)) return
    chargeLimitActual = Math.max(0, Math.min(100, v))
    chargeLimitReadOk = true
  }

  function chargeLimitIsActive(value) {
    if (root.chargeLimitReadOk) return root.chargeLimitActual === Number(value)
    return root.chargeLimitEnd === Number(value)
  }
  ...
  function setChargeLimit(value) {
    ...
    chargeLimitActual = Number(v)
    chargeLimitReadOk = true
    limitProc.command = ["asusctl", "battery", "limit", v]
      ...
      Column {
        ...
        Column {
          ...
          Row {
            ...
            readonly property real cellWidth: root.chargeLimits.length > 0
              ? (width - spacing * (root.chargeLimits.length - 1)) / root.chargeLimits.length
              : 0

            Repeater {
              model: root.chargeLimits
              Button {
                required property var modelData
                required property int index
                width: limitRow.cellWidth
                iconText: Model.chargeLimitIcon(modelData)
                iconSize: Style.font.title
                text: Model.chargeLimitLabel(modelData)
                fontSize: Style.font.bodySmall
                foreground: root.bar.foreground
                fontFamily: root.bar.fontFamily
                horizontalPadding: Style.spacing.controlPaddingX
                verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
                bordered: true
                active: root.chargeLimitIsActive(modelData)
                hasCursor: false
